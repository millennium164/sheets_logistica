"""
Validação nota × base (fluxo amigável).

Após escolher os arquivos Excel e as abas:
- filtro da base: automático, sem pop-up (mesmas colunas sugeridas no main.py);
  se o filtro esvaziar a base, segue com a base inteira;
- chaves de ligação: inferidas como no main.py ao abrir a tela de chaves
  (principal + fallback), sem interação;
- pares de validação: todos os pares com score ≥ 0.70, em mapeamento 1:1
  (primeira ocorrência por coluna), depois gera o Excel de saída.
"""
import pandas as pd
...
# ---------------- selecionar abas ----------------
aba_win = tk.Toplevel()
aba_win.title("Validação — passo 1: abas")
...
# ---------------- filtro inicial da base (automático; equivalência ao "Continuar" do main) ----------------
# Mesma heurística de colunas que preenchia os comboboxes na etapa 2 do main.py.
...
col_filtro_nota_auto = _coluna_filtro_preferida(
    df_nota.columns.tolist(),
    ["NOTA FISCAL", "NF", "NF_ENTRADA", "NUMERO NOTA"],
)
col_filtro_base_auto = _coluna_filtro_preferida(
    df_base.columns.tolist(),
    ["NOTA DE ENTRADA", "NF ENTRADA", "NF_ENTRADA", "NOTA FISCAL"],
)

if col_filtro_nota_auto and col_filtro_base_auto:
    try:
        df_base_filtrado, _qtd_vals, _antes, depois = filtrar_base_por_nota(
            df_nota,
            df_base,
            col_filtro_nota_auto,
            col_filtro_base_auto,
        )
        if depois > 0:
            df_base = df_base_filtrado
        # depois == 0: mantém df_base original, sem aviso
    except Exception as e:
        messagebox.showerror("Erro ao aplicar filtro automático", f"{type(e).__name__}: {e}")
        raise
# sem par identificável para filtro: base completa, sem aviso

# ---------------- chaves de ligação (automático: equivalente a Confirmar com sugestões) ----------------
sug_chaves = sugerir_chaves_por_unicidade(
    df_nota,
    df_base,
    limite=2,
    min_score_match=0.25,
    min_cobertura=0.20,
    min_unicidade=0.60,
    amostra=8000,
    top_debug=0,
...
if sug_chaves:
    for cn, cb, *_ in sug_chaves:
        if cn in df_nota.columns and cb in df_base.columns:
            chaves_nota.append(cn)
            chaves_base.append(cb)
if not chaves_nota:
    messagebox.showerror(
        "Chaves não identificadas",
        (
            "Não foi possível inferir automaticamente as colunas de ligação entre nota e base.\n"
            "Use o main.py para definir as chaves manualmente."
        ),
    )
    raise SystemExit

# ---------------- pares de validação (automático: score ≥ 0.70, mapeamento 1:1) ----------------
...
if not sugestoes:
    messagebox.showerror(
        "Sem pares para validar",
        (
            "Não foi possível inferir pares de colunas com score ≥ 0.70.\n"
            "Use o main.py para mapear colunas manualmente."
        ),
    )
    raise SystemExit
...
usados_b = set()
pares = []
for cn, cb, _sc in sugestoes:
    if cn in usados_n or cb in usados_b:
        continue
    if cn not in df_nota.columns or cb not in df_base.columns:
        continue
    usados_n.add(cn)
    usados_b.add(cb)
    pares.append((cn, cb))
...
if not pares:
    messagebox.showerror(
        "Mapeamento inválido",
        (
            "As sugestões automáticas não geraram nenhum par válido em mapeamento 1:1.\n"
            "Use o main.py para revisar o mapeamento."
        ),
    )
    raise SystemExit
